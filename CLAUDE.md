...
- **2026-06-02** — **A24/A25/A26 material-submission and shop-drawing fields** on `staging`, deployed to alpha. Added three new text-input appendix rows under clause 4.3(b) (page 22): "Start of Material Submission" (A24), "Complete all Material Submission" (A25), "Start of Submission of Shop Drawings" (A26). Master docx patched via `scripts/apply_master_4_3b_rows_patch.py` (clones A18 row, inserts 3 rows after it in the same table; idempotent; writes `.pre-4_3b.bak`). Migration `013_material_fields` shifts A19–A23 sort_orders +3 and inserts A24/A25/A26 at sort_orders 19/20/21 (idempotent NOT EXISTS guard). `seed_fields.py` updated for fresh deployments. **Gotcha (recurring):** alembic `version_num` is `varchar(32)` — keep revision IDs ≤ 32 chars. 32/33 pytest (1 documented flake), `tsc -b` clean.
