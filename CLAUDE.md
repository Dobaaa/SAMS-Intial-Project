...
2. Every master template change = new version. Old agreements keep FK to the exact version. **⚠️ Not actually true for the SCA docx pipeline as of 2026-08-04** — `Agreement.form_version_id`/`conditions_version_id`/`appendix_version_id` exist but are never consulted by `render_agreement_docx_to_pdf`; every render reads the single live `backend/masters/sca_master_v1.docx`, so a master-docx edit is picked up by *any* subsequent regeneration, including for an already-signed agreement, unless the caller happens to reuse the last stored `PDFOutput`. Phase 2 Package H (§13) adds a guard that blocks regeneration for completed/executed agreements, but does not implement true version-pinning.
3. Appendix is a diff document. Auto-populates from F/C. Admin can show/hide rows + add notes.
4. **Internal review is FLAT/parallel (2026-05-20 change).** On Submit for Review all four reviewer roles (PD, Accounts, OM, GM) see the agreement at once — no hierarchy. Each role either approves or adds a comment. Comments are **non-blocking**: visible to all roles, no bounce to Admin, no chain restart, the commenter's step stays pending. Admin can forward to the subcontractor only once **all four roles have approved** (`all_main_steps_approved` gate in `send_to_subcontractor`). The separate **resolution chain** (OM→GM, after subcontractor comments via `under_bgcc_revision`) is still sequential. (Superseded the old "all returns go to Admin / resubmit restarts the chain from PD" rule for the main review; `return_step`/`resubmit_agreement`/`/workflow/{id}/return` remain for the resolution path.) **⚠️ PLANNED REVERSAL — Phase 2, not yet built, see §13:** BGCC has requested going back to a sequential chain (Accounts→PD→OM→GM). Do not assume this rule is still accurate without checking whether Phase 2 Package A has landed.
...
## 13. Phase 2 — planned, not yet built

BGCC's Phase 2 wishlist (`~/Downloads/Approval process shall be step by step .md`, 14 items) has a plan approved 2026-08-04 but **zero implementation** as of this writing. Full plan with file:line references: `/home/seif/.claude/plans/home-seif-downloads-approval-process-sh-delegated-honey.md`.
...
- **A** — Reverse §4 rule #4 back to sequential: chain reorder to Accounts→PD→OM→GM, real ordering guard in `approve_step` (today's flat model has none), 3-button reviewer UI (Approved / Approved with comments / Rejected with comments) replacing today's Approve+Comment pair, wiring the already-built-but-unused `return_step`/`/workflow/{id}/return` into the frontend. "QS" in "Admin/QS→Accounts..." = the drafting Admin, not a new role — the dormant `quality_surveyor` DB role stays unused.
- **H** — Guard in `generate_agreement_pdf` blocking PDF regeneration for completed/executed agreements (closes the version-pinning gap noted in §4 rule #2). Land before F/G.
- **F** — Remove Commencement Date (A15 field + one clause-4.3 sentence); Time-for-Completion already satisfies "in days" (C08/A16/A17 are already text fields, no work needed there).
...
- **C** — Red-highlight admin-entered content in a new GM-only "View PDF" render mode (`highlight_admin_content` flag, default off — standard/archived PDF never changes).
