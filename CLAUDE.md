...
- **2026-05-11** — BGCC user-feedback round 1 (see `~/Downloads/SAMS-BGCC Comments-8.05.26.md`). 19 comments triaged; user confirmed scope via AskUserQuestion. Shipped on `staging`:
  ...
  - **Agreement wizard** (`AgreementCreate.tsx`): Back button on Steps 2-5; **Step 2 drops F02-F08** (duplicates of Step 1's project/subcontractor inputs — values sync via `buildStep1FieldSync` after createDraft / saveStep1Edit). **Step 4 drops the always-visible A-field override sections**; per-row inline Edit in `AppendixBuilder` is the single edit surface. **Step 1 subcontractor picker** with debounced search pre-fills name / PO box / TL / contact / email / phone / address (NOT the contract price).
  - **Workflow Review** side-by-side: new `AppendixView` component renders the appendix as a structured HTML table next to the SCA PDF iframe in a 2-col grid, so PD/Accounts/OM/GM compare both documents without switching tabs (comments #1 + #2).
  - **Resolution** (`CommentsResolution.tsx`): added `original_clause_text` input on create; replaced the wide table with per-row cards that show **Original SCA Clause** (amber) vs **Second Party Comment** (sky) side-by-side per comment, plus AI / PD / OM / Final response panels beneath (comment #18). Backend `routers/resolution.py` and the `/agreements/{id}/resolution-sheet` mirror payload updated.
  - **Number formatting** (comments #11/#12): new `frontend/src/lib/formatNumber.ts` (en-US locale, thousands comma, dot decimal, 0 or 2 dp). Applied in AppendixBuilder, AppendixView, Step 5 review. **PDF**: new `_format_money` helper registered as Jinja filter `money`; `_render_master_with_values` now formats numeric fields automatically when their master_fields.input_type=='number', so `{{F08}}` in template HTML renders "1,000,000.00". `appendix.html` uses `| money` on A07/A09/A10/A20.
  - **C14 wiring**: `FieldInput` adds a dropdown for `C14`; `appendix.html` row "Performance Security" now includes a sub-line "Type: {C14}" under the 10% amount.
  - **AppLayout**: added BGCC brand block (gradient logo tile + "BHATIA / General Contracting Co." text) to the top nav (comment #3).
  - **Dashboard**: Edit button now shows on `under_internal_review` too (comment #4 admin-edits-mid-review). Delete button (red) shows for the same statuses pre-GM-approval, with browser confirm (comment #5).
  - **Masters page**: new collapsible help panel at the top explaining versioning, fields, `{{FIELD_ID}}` placeholders, and appendix rendering (comment #17).
  - **Verification**: `npx tsc -b` EXIT=0; `npm run test:run` 13/13 pass; `npm run build` clean; `pytest` 22/22 pass.
  - **Open items still needing user input**:
    - **Comments #1+#2 content seeding**: user committed to providing the full 42-page Word/HTML content; once received, drop into `backend/seeds/form_master.html` + `conditions_master.html` and re-run `python -m scripts.seed_master_content`. The side-by-side WorkflowReview layout is already shipped and will render the longer content automatically.
    - **Comment #9 (notifications)**: user chose to configure SMTP on the alpha VPS rather than build in-app notifications. To-do (manual): set `SMTP_HOST`, `SMTP_USER`, `SMTP_PASS` in `/var/www/sams/backend/.env` and restart supervisor. Code already logs and silently swallows transport failures, so once SMTP is real, emails fire on submit / return / approve / send-to-subcontractor.
